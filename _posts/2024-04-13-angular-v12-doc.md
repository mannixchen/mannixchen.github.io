...
- **服务作用域**：在 Angular 中，服务通常在 `providers` 数组中注册。这个注册可以发生在应用的根模块（`AppModule`）、任何其他模块，或者组件级别。
- 模块加载方式的影响：
  - **立即加载的模块**：如果一个服务是在立即加载的模块中提供的，该服务将可用于整个应用，包括所有父模块和子模块。这是因为当 Angular 启动时，所有立即加载的模块都被加载并编译，它们的提供的服务会被添加到同一个注入器（injector）中。
  - **懒加载的模块**：对于懒加载模块，它们拥有独立的注入器。在这种情况下，懒加载模块中提供的服务只对该模块及其动态加载的子模块可见。

#### 父模块使用子模块的服务：

如果子模块是立即加载的，并且在子模块中提供了服务，那么这些服务可以被任何导入该子模块的其他模块使用（包括父模块），因为这些服务已经被添加到应用的根注入器中。这样，即便服务没有在 `exports` 中列出，父模块也可以使用子模块中提供的服务。**记住，`exports` 数组仅用于组件、指令和管道，而不用于服务。**
...
* 当Angular需要创建一个组件的实例时，它会**首先检查该组件的构造函数**，**看看组件是否声明了任何依赖（即构造函数的参数**）。如果构造函数有参数，Angular会根据参数的**类型**（在上述示例中是`LogService`）到自己的**注入器**中查找对应的服务提供者。(关于这个构造函数的参数, 服务知识其一种类型。其余的可能还有: **路由信息ActivatedRoute, DOM 引用ElementRef**等)
